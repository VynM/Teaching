...
\usepackage{enumitem}
\setlist[enumerate,1]{label=\arabic*., leftmargin=*, itemsep=0.8em}
...
\newcommand{\sectiontitle}[1]{%
  \medskip
  {\noindent\large\bfseries\color{blue!35!black} #1}\par
  \smallskip
}
...
\begin{enumerate}
  ...
  \item Give a spanning tree of $G$.
...
\sectiontitle{II. Core techniques}
...
\item Apply Dijkstra's algorithm to the weighted graph below, starting at
...
\draw (s)--node[edge label] {4} (a);
\draw (s)--node[edge label] {2} (b);
\draw (a)--node[edge label] {1} (b);
\draw (a)--node[edge label] {5} (c);
\draw (b)--node[edge label] {8} (c);
\draw (b)--node[edge label] {9} (d);
\draw (c)--node[edge label] {2} (d);
\draw (c)--node[edge label] {6} (t);
\draw (d)--node[edge label] {3} (t);
\end{tikzpicture}
\end{center}

How do the shortest path and distance from $s$ to $t$ change if the
weight of $ct$ is changed from $6$ to $2$?

\item Let $G$ be a graph of order $n$.
\begin{enumerate}
  \item Express $\deg_{\comp G}(v)$ in terms of $\deg_G(v)$.
  \item Prove that $A(\comp G)=J-I-A(G)$.
  \item If $G$ and $\comp G$ are both $k$-regular, determine $n$ in
  terms of $k$, and show that $k$ must be even.
\end{enumerate}

\sectiontitle{III. Proof and synthesis}
