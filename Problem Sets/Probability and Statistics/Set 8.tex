...
\item If $0 < p < 1$ and $q = 1 - p$, find the mgf of the random variable $X$ with pmf $f(x) = p^{x - 1} q$, $x = 1, 2, \ldots$, and hence find $\EV[X]$ and $\Var[X]$.

\item If $\EV[X^n] = 2^n (n + 1)!$, $n = 0, 1, \ldots$, then find the mgf of $X$.
...
\item If $X$ is a random variable with pdf $f(x) = e^{-2|x|}$, find the mgf of $X$, and hence compute $\EV[X]$ and $\Var[X]$.

\item If $X \sim U[a, b]$, compute $\EV[X^n]$ using $M_X(t)$. Hence show that if $X \sim U[-a, a]$, then $\EV[X^{2n}] = \dfrac{a^{2n}}{2n + 1}$.
