...
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	...
	\path[->]
	(A) edge [loop above]	node {\texttt 0}	()
	(A) edge [bend left]	node {\texttt 1}	(B)
	(B) edge [loop above]	node {\texttt 0}	()
	(B) edge [bend left]	node {\texttt 1}	(A);
\end{tikzpicture}
\end{center}
\end{Example}
...
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	...
	\path[->]
	(A) edge [loop above]	node {\texttt 1}	()
	(A) edge [bend left]	node {\texttt 0}	(B)
	(B) edge [loop above]	node {\texttt 1}	()
	(B) edge [bend left]	node {\texttt 0}	(A);
\end{tikzpicture}
\end{center}
}
...
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	...
	\path[->] 
	(q0) edge [] node {\texttt a}	(A)
	(q0) edge [] node [swap] {\texttt b} (T)
	(A) edge []	node {\texttt a}	(AA)
	(A) edge [] node {\texttt b} (T)
	(AA) edge []	node {\texttt b}	(AAB)
	(AA) edge [] node {\texttt a} (T)
	(AAB) edge [loop above]	node {\texttt a, \texttt b}	()
	(T)	edge [loop below] node {\texttt a, \texttt b} ();
\end{tikzpicture}
\end{center}
}
...
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	...
	\path[->] 
	(q0) edge [] node {\texttt 1}	(q1)
	(q1) edge []	node {\texttt 1}	(q11)
	(q11) edge []	node {\texttt 0}	(q110)
	(q0) edge [loop above]	node {\texttt 0, \texttt 1}	()
	(q110)	edge [loop above] node {\texttt 0, \texttt 1} ();
\end{tikzpicture}
\end{center}
\end{Example}
...
If $M$ has epsilon transitions, then in addition to the above, $\delta'\pqty{ \{q_1, \ldots, q_k\}, s }$ will contain, in addition to the states described above, the states of $M$ that can be reached from each state in $\delta(q_i, s)$ via an epsilon transition.
...
\end{tikzpicture}
\end{center}
Here, the initial state is $q_0 = A$, and the set of accept states is $F = \{D\}$. Now, the equivalent DFA $D_M$ will $2^4 = 16$ states, namely $\varnothing, \{A\}, \{B\}, \{C\}, \{D\}, \{A, B\}, \{A, C\}, \ldots, \{A, B, C, D\}$. For the sake of simplicity, we will denote these as $\varnothing, A, B, C, D, AB, AC, \ldots, ABCD$.
...
\item Next, $\delta'(A, 0) = \delta(A, 0) = \{A\}$ (written as just $A$) and $\delta'(A, 1) = \delta(A, 1) = \{A, B\}$ (written as just $AB$).
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {A};
	\node[state, minimum size = 10mm] (AB) [right = of A] {AB};
	\path[->] 
	(A) edge [] node {\texttt 1}	(AB)
	(A) edge [loop above]	node {\texttt 0}	();
\end{tikzpicture}
\end{center}

\item Now, $\delta'(AB, 0) = \delta(A, 0) \cup \delta(B, 0) = \{A\} \cup \varnothing = \{A\}$ (i.e. $A$) and $\delta'(AB, 1) = ABC$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	...
	\path[->] 
	(A)		edge [bend left] 	node {\texttt 1}	(AB)
	(AB)	edge [bend left] 	node {\texttt 0}	(A)
	(AB)	edge [] 			node {\texttt 1}	(ABC)
	(A)		edge [loop above]	node {\texttt 0}	();
\end{tikzpicture}
\end{center}

\item Similarly, $\delta'(ABC, 0) = AD$ and $\delta'(ABC, 1) = ABC$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {A};
	\node[state, minimum size = 11mm] (AB) [right = of A] {\small AB};
	...
	(AB)	edge [bend left] 	node {\texttt 0}	(A)
	(AB)	edge [] 			node {\texttt 1}	(ABC)
	(ABC)	edge [] 			node {\texttt 0}	(AD)
	(A)		edge [loop above]	node {\texttt 0}	()
	(ABC)	edge [loop above]	node {\texttt 1}	();
\end{tikzpicture}
\end{center}

\item Similarly, $\delta'(AD, 0) = AD$ and $\delta'(AD, 1) = ABD$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {A};
	\node[state, minimum size = 11mm] (AB) [right = of A] {\small AB};
	...
	(AB)	edge [] 			node {\texttt 1}	(ABC)
	(ABC)	edge [] 			node {\texttt 0}	(AD)
	(AD)	edge [] 			node {\texttt 1}	(ABD)
	(A)		edge [loop above]	node {\texttt 0}	()
	(ABC)	edge [loop above]	node {\texttt 1}	()
	(AD)	edge [loop above]	node {\texttt 0}	();
\end{tikzpicture}
\end{center}

\item Similarly, $\delta'(ABD, 0) = AD$ and $\delta'(ABD, 1) = ABCD$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {A};
	\node[state, minimum size = 11mm] (AB) [right = of A] {\small AB};
	...
	(AB)	edge [] 			node {\texttt 1}	(ABC)
	(ABC)	edge [] 			node {\texttt 0}	(AD)
	(AD)	edge [bend left] 	node {\texttt 1}	(ABD)
	(ABD)	edge [bend left] 	node {\texttt 0}	(AD)
	(ABD)	edge [] 			node {\texttt 1}	(ABCD)
	(A)		edge [loop above]	node {\texttt 0}	()
	(ABC)	edge [loop above]	node {\texttt 1}	()
	(AD)	edge [loop above]	node {\texttt 0}	();
\end{tikzpicture}
\end{center}
\item Finally, $\delta'(ABCD, 0) = AD$ and $\delta'(ABCD, 1) = ABCD$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {A};
	\node[state, minimum size = 11mm] (AB) [right = of A] {\small AB};
	...
	(ABD)	edge [bend left] 	node {\texttt 0}	(AD)
	(ABD)	edge [] 			node {\texttt 1}	(ABCD)
	(ABCD)	edge [bend left = 50] 	node {\texttt 0}	(AD)
	(A)		edge [loop above]	node {\texttt 0}	()
	(ABC)	edge [loop above]	node {\texttt 1}	()
	(AD)	edge [loop above]	node {\texttt 0}	()
	(ABCD)	edge [loop above]	node {\texttt 1}	();
\end{tikzpicture}
\end{center}
