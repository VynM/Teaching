...
\begin{Example}
Let $S = \{1, 2, 3, 4, 5\}$. Then $P = \{\{1, 3, 5\}, \{2, 4\}\}$ and $Q = \{\{1, 4\}, \{2\}, \{3, 5\}\}\}$ are two different partitions of $S$.
\end{Example}
...
A \newterm{triangle} in a graph is a subgraph consisting of three mutually adjacent vertices -- i.e. an instance of $K_3$ as a subgraph.

\begin{Theorem}[Ramsey]\label{thm:R(3,3)}
If $G$ is any graph of order at least six, then either $G$ or $\overline G$ contains a triangle.
\end{Theorem}

\begin{proof}
Consider a graph $G$ of order $n \ge 6$, and let $v \in V(G)$. Since $\deg_G v + \deg_{\overline G} v = n - 1 \ge 5$, in either $G$ or $\overline G$, $v$ has degree at least $3$.

Without loss of generality, suppose that $v$ is adjacent to three vertices $x$, $y$, $z$ in $G$. Now, if any two of $x$, $y$, and $z$ are adjacent to each other in $G$, then, together with $v$, they form a triangle in $G$. Otherwise, $xyz$ is a triangle in $\overline G$.
\end{proof}

\begin{Definition}
For positive integers $m$ and $n$, the \newterm{Ramsey number} $R(m, n)$ is the least positive integer such that if $G$ is any graph of order $R(m, n)$, then either $G$ contains $K_m$ or $\overline G$ contains $K_n$ as a subgraph -- i.e. $G$ contains either $K_m$ or $\overline K_n$ as an induced subgraph.
\end{Definition}

Observe that there exists a graph $G$ of order $5$ such that neither $G$ nor $G$ contains a triangle. Therefore, by Theorem~\ref{thm:R(3,3)}, $R(3, 3) = 6$.
...
\subsubsection{Complete Bipartite Graphs}\label{subsec:Krs}

The \newterm{complete bipartite graph} $K_{r, s}$ is the bipartite graph with bipartition $(V_1, V_2)$, where $|V_1| = r$, $|V_2| = s$, and every vertex in $V_1$ is adjacent to every vertex in $V_2$. The complete bipartite graph $K_{1, n}$ is the \newterm{star graph} of order $n + 1$.
...
\begin{Exercise}
Suppose $G$ is a bipartite graph with bipartition $(V_1, V_2)$, and furthermore, assume that the vertices are ordered in such a way that the first $r$ vertices are in $V_1$ and the next $s$ vertices are in $V_2$. Show that the adjacency matrix of $G$ is given by the partitioned matrix
\begin{equation*}
\mathcal A(G) = \begin{bmatrix}
0_r & B \\
B^T & 0_s
\end{bmatrix}
\end{equation*}
where $B$ is a $(0, 1)$-matrix of order $r \times s$. What is $B$ when $G$ is $K_{r, s}$?
\end{Exercise}
