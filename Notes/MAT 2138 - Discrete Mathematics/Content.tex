...
\item Now, $\delta'(AB, \texttt 0) = \delta(A, \texttt 0) \cup \delta(B, \texttt 0) = \{A\} \cup \varnothing = \{A\}$ (i.e.\ $A$) and $\delta'(AB, \texttt 1) = ABC$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
...
\item Similarly, $\delta'(ABC, \texttt 0) = AD$ and $\delta'(ABC, \texttt 1) = ABC$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {$A$};
	\node[state, minimum size = 11mm] (AB) [right = of A] {\small $AB$};
...
\item Similarly, $\delta'(AD, \texttt 0) = AD$ and $\delta'(AD, \texttt 1) = ABD$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {$A$};
	\node[state, minimum size = 11mm] (AB) [right = of A] {\small $AB$};
...
\item Similarly, $\delta'(ABD, \texttt 0) = AD$ and $\delta'(ABD, \texttt 1) = ABCD$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {$A$};
	\node[state, minimum size = 11mm] (AB) [right = of A] {\small $AB$};
...
\item Finally, $\delta'(ABCD, \texttt 0) = AD$ and $\delta'(ABCD, \texttt 1) = ABCD$.
\begin{center}
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	\node[state, initial, minimum size = 10mm] (A) {$A$};
	\node[state, minimum size = 11mm] (AB) [right = of A] {\small $AB$};
...
%	\node[state, accepting, minimum size = 6mm] (C) [right = of B] {C};
	\path[->] 
	(ABC) edge []				node {\texttt b}	(BC)
%	(BC) edge []				node {$\varepsilon$}	(C)
	(ABC) edge [loop above]	node {\texttt a}	();
%	(B) edge [loop above]	node {\texttt b}	()
%	(C) edge [loop above]	node {\texttt c}	();
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[> = stealth, shorten > = 1pt, node distance = 2cm, on grid, auto, initial text =]
	...
	\path[->] 
	(ABC) edge []				node {\texttt b}	(BC)
	(ABC) edge [bend right]		node [swap] {\texttt c}	(C)
	(BC) edge []				node {\texttt c}	(C)
	(BC) edge []				node [swap] {\texttt a}	(T)
	(C) edge []				node {\texttt a, \texttt b}	(T)
	(ABC) edge [loop above]	node {\texttt a}	()
	(BC) edge [loop above]	node {\texttt b}	()
	(C) edge [loop above]	node {\texttt c}	();
\end{tikzpicture}
\end{center}
\end{Example}
...
\item The \newterm{Kleene star} of $L_1$ is $L_1^*$, the language of all strings obtained by concatenating any finite number of strings of $L_1$ together.
\begin{align*}
...
\item All binary strings containing an odd number of $1$s.
\end{enumerate}
}
